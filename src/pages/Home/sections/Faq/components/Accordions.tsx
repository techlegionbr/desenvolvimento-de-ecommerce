...
const Accordions = ({ accordions }: PropsAccordions) => {
  ...
  const accodionsMap = Object.fromEntries(
    accordions.map((_accordion, index) => [(index + 1).toString(), false])
  )

  const [accordionsState, setAccordionsState] = useState(accodionsMap)
